...
class SiteFormConfig(AppConfig):
    ...
    def ready(self):
        ...
        try:
            self.json_data = load_json_from_google_drive(diseases_file_id)
            print("JSON успешно загружен из Google Диска.")
        except Exception as e:
            print(f"Ошибка при загрузке JSON: {e}")

        
        analysis_file_id = settings.GOOGLE_DRIVE_ANALYSIS_JSON_FILE_ID
        try:
            self.json_data = load_json_from_google_drive(analysis_file_id)
            print("JSON успешно загружен из Google Диска.")
        except Exception as e:
            print(f"Ошибка при загрузке JSON: {e}")
